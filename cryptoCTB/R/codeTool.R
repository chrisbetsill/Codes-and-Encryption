...
codeDistance <- function(C) {
  ...
  for(i in (j+1):(nrow(C))){
    vec2 <- C[i, ]
    dist <- hammingDistance(vec1, vec2)
    if(dist < distance)
      distance <-  dist
  }
  }
...
generateCode <- function(B) {
 ...
 for(i in 1:numCombos){
   comb <- as.vector(as.numeric(intToBits(i))[1:numRows])
   lc <- (comb %*% B) %% 2
   mVec <- c(mVec, lc)
 }
 return(unique(matrix(mVec, nrow=numCombos, byrow=TRUE)))
}
...
isPerfect <- function(C) {
  ...
  denom <- 0
  for(i in 0:t){
    denom <- denom + choose(numCols, i)
  }
  
  if(2^numCols/denom == numRows)
    return(TRUE)
  else 
    return(FALSE)
}